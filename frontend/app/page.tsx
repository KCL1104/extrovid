...
export default function Dashboard() {
  ...
  async function create() {
    ...
    setError(null);
    try {
      await createProject({ title: title.trim(), aspect_ratio: aspect, target_duration_sec: duration });
      setTitle("");
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-16 sm:py-24">
      <header className="rise">
        ...
        <h1 className="title mt-3 text-6xl text-fg sm:text-7xl">
          extro<span className="italic text-accent">vid</span>
        </h1>
        <p className="mt-4 max-w-xl leading-relaxed text-muted">
          Define the intent. The system writes the script, previsualizes the look, generates each shot, and
          assembles the cut.
        </p>
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="flex-1">
            ...
            <input
              ...
              disabled={creating}
              placeholder="Coffee brand teaser"
              className={`mt-1 w-full rounded-[var(--radius)] border border-border bg-bg-soft px-3 py-2 text-fg outline-none placeholder:text-faint focus:border-accent/60 ${focusRing}`}
            />
          </label>
          <label>
            ...
            <select
              ...
              className={`mt-1 w-full rounded-[var(--radius)] border border-border bg-bg-soft px-3 py-2 font-mono text-sm text-fg outline-none focus:border-accent/60 sm:w-24 ${focusRing}`}
            >
              {ASPECTS.map((a) => (
      ...
      <section className="mt-10">
        ...
        {projects === null ? (
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {projects.map((p, i) => (
              <Link key={p.id} href={`/projects/${p.id}`} className="group rise block" style={{ animationDelay: `${i * 40}ms` }}>
