...
> build all green. **P3a DONE** (36284fa — caps→~20min + per-project budget gate). **P3b DONE**
> (135f557 — LONG chapter/act layer: Act table + Scene.act_id, outline agent, /plan/outline,
> act-structured script, chapter accordion in Review). **P3c (still-vs-motion) DEFERRED** — its
> cut-side freeze-frame rendering restructures the (ShotVersion, Shot)-coupled ffmpeg assembly
> (high regression risk); do it as a careful isolated follow-up (render the still→clip at
> generation time so the cut pipeline stays untouched). Both original asks (multi-length +
> review gate) are fully met by P0–P3b. Design locked via research workflow `wf_1c125d7d`.
